...
import XMonad.Hooks.DynamicLog (dynamicLogWithPP, wrap, shorten, xmobarPP, xmobarColor, PP(..))
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
   ...
   -- launch gmrun
	...
	-- kill xmobar process
	, ((modm, xK_b), spawn "ps aux | grep \"xmobar \" | grep -v \"grep\" | awk '{print $2}' | xargs kill -9")
...
mySpacing = spacingRaw False 		-- False=Apply even when single window
				(Border 6 6 6 6) -- Screen border size top btm rght lft
				True		-- Enable screen border
				(Border 6 6 6 6) -- Window border size
				True		-- Enable window border
